...
public class ItemTag implements ObjectTag, Adjustable, FlaggableObject {
    ...
    public static void register() {
        ...
        tagProcessor.registerMechanism("max_durability", true, (object, mechanism) -> {
            ItemStack bukkitItem = object.getItemStack();
            ItemMeta meta = bukkitItem.getItemMeta();

            if (!(meta instanceof Damageable damageable)) {
                mechanism.echoError("This item does not support max_durability modification!");
                return;
            }

            if (!mechanism.hasValue()) {
                damageable.setMaxDamage(null);
                bukkitItem.setItemMeta(damageable);
                return;
            }
