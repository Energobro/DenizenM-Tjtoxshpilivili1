...
public class AdvancementHelperImpl extends AdvancementHelper {
    ...
    public void register(com.denizenscript.denizen.nms.util.Advancement advancement) {
        ...
        setManagerAdvancements(mapBuilder.build());
        ...
        if (!advancement.hidden && advancement.parent != null) {
            PacketHelperImpl.broadcast(new ClientboundUpdateAdvancementsPacket(false, List.of(new ClientboundUpdateAdvancementsPacket.PositionedAdvancement(nmsAdvancementHolder, advancement.xOffset, advancement.yOffset)), Set.of(), Map.of(), false));
        }
    }
    ...
    public void grantPartial(com.denizenscript.denizen.nms.util.Advancement advancement, Player player, int len) {
        ...
        if (advancement.temporary) {
            ...
            PacketHelperImpl.send(player, new ClientboundUpdateAdvancementsPacket(false, List.of(positionedFor(nmsAdvancement)), Set.of(), Map.of(nmsAdvancement.id(), progress), false));
        }
        else {
    ...
    public void grant(com.denizenscript.denizen.nms.util.Advancement advancement, Player player) {
        ...
        if (advancement.temporary) {
            ...
            PacketHelperImpl.send(player, new ClientboundUpdateAdvancementsPacket(false, List.of(positionedFor(nmsAdvancement)), Set.of(), Map.of(nmsAdvancement.id(), progress), true));
        }
        else {
            AdvancementHolder nmsAdvancement = getNMSAdvancementManager().advancements.get(CraftNamespacedKey.toMinecraft(advancement.key));
    ...
    /** 'advancements' on the manager is final in 26.3, so a replacement map goes in through a final setter. */
    public static final MethodHandle ADVANCEMENTS_SETTER = ReflectionHelper.getFinalSetter(ServerAdvancementManager.class, "advancements");

    private static void setManagerAdvancements(Map<Identifier, AdvancementHolder> replacement) {
        try {
            ADVANCEMENTS_SETTER.invoke(getNMSAdvancementManager(), replacement);
        }
        catch (Throwable ex) {
            Debug.echoError(ex);
        }
    ...
    /**
     * 26.3 moved an advancement's tree position out of DisplayInfo and into the packet, next to the advancement itself.
     * The node already carries the position the tree worked out, so it is read back from there rather than stored twice.
     */
    private static ClientboundUpdateAdvancementsPacket.PositionedAdvancement positionedFor(AdvancementHolder holder) {
        AdvancementNode node = getNMSAdvancementManager().tree().get(holder.id());
        return node != null ? ClientboundUpdateAdvancementsPacket.PositionedAdvancement.fromNode(node)
                : new ClientboundUpdateAdvancementsPacket.PositionedAdvancement(holder, 0f, 0f);
    }
}
