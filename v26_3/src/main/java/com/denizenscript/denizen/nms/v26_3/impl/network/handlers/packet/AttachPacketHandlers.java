                ...
                if (attMap.attached.isValid() && att != null) {
                    ...
                    if (att.positionalOffset != null) {
                        ...
                        if ((isRotate && att.offsetRelative) || forceTele || offX < Short.MIN_VALUE || offX > Short.MAX_VALUE
                            ...
                            // The offset used to be three short fields poked in by reflection. 26.3 carries it as one
                            // VecDelta, and a packet cannot be edited after the fact, so it is rebuilt with the delta wanted.
                            VecDelta newDelta = new VecDeltaCodec().tryEncode(new Vec3(moveNeeded.getX(), moveNeeded.getY(), moveNeeded.getZ()));
                            if (newDelta == null) {
                                newDelta = VecDelta.ZERO;
                            }
                            pNew = isRotate
                                    ? new ClientboundMoveEntityPacket.PosRot(newId, newDelta, EntityAttachmentHelper.compressAngle(yaw), EntityAttachmentHelper.compressAngle(pitch), e.onGround())
                                    : new ClientboundMoveEntityPacket.Pos(newId, newDelta, e.onGround());
                            if (NMSHandler.debugPackets) {
